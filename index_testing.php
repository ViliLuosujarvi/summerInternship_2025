...
.cabins{display:flex;gap:1.5rem;margin-bottom:2rem;flex-wrap:wrap;justify-content:center;}
.cabin-card{background:#fff;padding:1rem;border-radius:12px;box-shadow:0 4px 8px rgba(0,0,0,0.15);flex:1 1 30%;text-align:center;cursor:pointer;transition: transform 0.2s, box-shadow 0.2s;max-width:300px;}
...
/* Modal */
#cabinOverlay{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.4);z-index:9998;overflow-y:auto;}
#cabinDetails{position:relative;margin:5% auto;max-width:700px;background:#fff;padding:2rem;border-radius:12px;box-shadow:0 8px 25px rgba(0,0,0,0.3);z-index:9999;}
...
.thumbnails{display:flex;gap:0.5rem;margin-bottom:1rem;flex-wrap:wrap;justify-content:center;}
...
if($conn->connect_error){ echo "<p>DB connection error</p>"; }
else{
  ...
  if($result && $result->num_rows>0){
    ...
    while($row=$result->fetch_assoc()){
      echo "<tr><td>{$row['id']}</td><td>{$row['cabin']}</td><td>{$row['checkin']}</td><td>{$row['checkout']}</td><td>{$row['total_price']}</td><td>{$row['payment_status']}</td><td>{$row['created_at']}</td></tr>";
    }
    echo "</table>";
...
<footer>© 2025 Siiranmäki Cabins. All rights reserved.</footer>

<script>
// Cabin Data
const cabins={
helmi1:{title:"Helmi 1",description:"A cozy cabin by Käränkävaara ridge. Sleeps 4.",priceTiers:{'2':75,'3-5':70,'6+':50},cleaning:100,linen:25,images:["https://media.houseandgarden.co.uk/photos/63a1a9b588e2d802928c6499/2:3/w_2000,h_3000,c_limit/MFOX7961.jpg","https://hips.hearstapps.com/hmg-prod/images/clx100122welldargenzio-002-2-66d768fc6c262.jpg","https://stofferhome.com/cdn/shop/collections/Screen_Shot_2023-01-17_at_2.25.55_PM_3024x.png?v=1673983797","https://i.pinimg.com/736x/8b/f2/0d/8bf20dad9de73912e8f79ab827f5da4b.jpg"]},
helmi2:{title:"Helmi 2",description:"Rustic cabin with sauna, sleeps 6.",priceTiers:{'2':120,'3-5':110,'6+':70},cleaning:100,linen:25,images:["https://cf.bstatic.com/xdata/images/hotel/max1024x768/615536116.jpg?k=1105a0cd9fd25cd7ebe53a200226d44a240536de1369da10089033a61471f2b9&o=&hp=1","https://cdn.mos.cms.futurecdn.net/9AW7pCmj5LmquUGiXLRhUW.jpg","https://blog.canadianloghomes.com/wp-content/uploads/2022/02/log-cabin-living-room.jpg","https://i.pinimg.com/736x/3a/c6/80/3ac6805b5b0a3fc46b68366a793b418a.jpg"]},
helmi3:{title:"Helmi 3",description:"Modern cabin with fireplace, sleeps 6.",priceTiers:{'2':110,'3-5':100,'6+':60},cleaning:100,linen:25,images:["https://images.fineartamerica.com/images/artworkimages/mediumlarge/1/cabin-at-the-lake-thomas-nay.jpg","https://gallery.streamlinevrs.com/units-gallery/00/05/CD/image_167989129.jpeg","https://gallery.streamlinevrs.com/units-gallery/00/0C/38/image_165319104.jpeg","https://stayovernow.com/wp-content/uploads/2024/01/image_163623794-e1704379674682.webp"]}
};

// Modal Elements
const cabinCards=document.querySelectorAll('.cabin-card');
const cabinOverlay=document.getElementById('cabinOverlay');
...
cabinCards.forEach(card=>{
  card.addEventListener('click',()=>{
    ...
    currentIndex=0;
    cabinImage.src=currentImages[0];
    updateThumbnails();
    cabinOverlay.style.display='block';
...
function updateThumbnails(){
  ...
  currentImages.forEach((img,i)=>{
    ...
    thumb.addEventListener('click',()=>{
      currentIndex=i;
      cabinImage.src=currentImages[i];
      updateThumbnails();
    });
...
// Close
closeDetails.addEventListener('click',()=>{cabinOverlay.style.display='none';});
cabinOverlay.addEventListener('click',(e)=>{if(e.target===cabinOverlay)cabinOverlay.style.display='none';});
...
function getNights(ci,co){return ci&&co?(new Date(co)-new Date(ci))/(1000*60*60*24):NaN;}
function pickPrice(pt,n){if(n>=6)return pt['6+'];if(n>=3&&n<=5)return pt['3-5'];return pt['2'];}
function calculatePrice(){const k=form.cabin.value;const n=getNights(form.checkin.value,form.checkout.value);const p=parseInt(form.people.value)||1;const c=form.cleaning.checked;const l=form.linen.checked;if(!k||isNaN(n)||n<2){totalPriceEl.textContent="0";return;}let total=pickPrice(cabins[k].priceTiers,n)*n;if(c)total+=cabins[k].cleaning;if(l)total+=cabins[k].linen*p;totalPriceEl.textContent=total;}
form.addEventListener('input',calculatePrice);
form.addEventListener('submit',(e)=>{const n=getNights(form.checkin.value,form.checkout.value);if(isNaN(n)||n<2){e.preventDefault();alert('Minimum stay is 2 nights.');return false;}confirmation.innerHTML="<p>Sending reservation to server…</p>";});
</script>
</body>
</html>
