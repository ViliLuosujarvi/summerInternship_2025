...
<style>
  ...
  #closeDetails {
    ...
    background:#d9534f; border:none; color:white; font-weight:bold;
    padding:0.5rem 1rem; border-radius:6px; cursor:pointer; transition:background 0.3s;
  }
  ...
  #cabinDetails::before { content:''; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.4); z-index:-1; }
...
function getNights(checkinStr,checkoutStr){
  ...
  const diff=(new Date(checkoutStr)-new Date(checkinStr))/(1000*60*60*24);
  return diff;
}
