...
<head>
  ...
  <style>
    ...
    .cabins { display:flex; gap:1.5rem; justify-content:center; flex-wrap:wrap; margin-bottom:2rem; }
    .cabin-card { background:#fff; padding:1rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.1); flex:1 1 30%; max-width:300px; text-align:center; cursor:pointer; transition: transform 0.2s; }
    ...
    @media(max-width:480px){ .cabin-card { flex:1 1 100%; } }
  </style>
</head>
<body>
  <header>
  ...
  <script>
    ...
    cabinCards.forEach(card=>{
      card.addEventListener('click',()=>{
        ...
        const c = cabins[key];
        cabinTitle.textContent = c.title;
        cabinDescription.textContent = c.description;
        detailCleaning.textContent = c.cleaning;
        detailLinen.textContent = c.linen;
    ...
    function pickPrice(priceTiers,n){ if(n>=6)return priceTiers['6+']; if(n>=3)return priceTiers['3-5']; return priceTiers['2']; }
